	...
	function lafka_register_required_plugins() {
		...
		$plugins = array(
				...
				array(
						...
						'required' => false, // If false, the plugin is only 'recommended' instead of required
						'version' => '6.5.15', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
						'force_activation' => false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
						'force_deactivation' => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins
				),
				array(
...
if (!function_exists('lafka_enqueue_scripts_and_styles')) {
	...
	function lafka_enqueue_scripts_and_styles() {
		...
		wp_register_script('lafka-google-maps', 'https://maps.googleapis.com/maps/api/js?'.( lafka_get_option('google_maps_api_key') ? 'key='.lafka_get_option('google_maps_api_key').'&' : '' ).'sensor=false', array('jquery'), false, true);
